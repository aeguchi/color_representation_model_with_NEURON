...
class Controller(object):
    def __init__(self,dim_stim,dim):
        ...
        initRandMinRange = 0.3 
        initRandMaxRange = 0.7
        ...
        for y in range(self.variables.dim_stim):
            for x in range(self.variables.dim_stim):
                ...
                self.r_stims[y][x].noise = 0#0.1
                self.variables.delay_r[y][x] = random();
                self.g_stims[y][x] = h.NetStim(0.5)
                self.g_stims[y][x].noise = 0#0.1
...
#         print "shift=" + str([LtoL4_mod, C1toL4_mod, C2toL23_mod])      
    ...
    def outputFR_trans(self,r,g,b,itr):
        ...
        f = open(self.resultFolderName+"/L23_FR_"+str(itr)+"_"+str([r, g, b])+".txt", "a")
        ...
        f = open(self.resultFolderName+"/L4_FR_"+str(itr)+"_"+str([r, g, b])+".txt", "a")
        for y in range(self.variables.dim):
            for x in range(self.variables.dim):
                index = y*self.variables.dim+x
        ...
        f = open(self.resultFolderName+"/L5_FR_"+str(itr)+"_"+str([r, g, b])+".txt", "a")
        for y in range(self.variables.dim):
            for x in range(self.variables.dim):
                index = y*self.variables.dim+x
