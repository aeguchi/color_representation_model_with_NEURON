...
#define _tqitem &(_ppvar[2]._pvoid)
 static _net_receive();
 extern int nrn_netcon_argslist();
 typedef (*_Pfrv)();
 ...
 add_nrn_fornetcons(_mechtype, _fnc_index);
 	...
 	ivoc_help("help ?1 ExpSynSTDP /home/aeguchi/Documents/workspace/SeriousDevelopment/Program_repo/x86_64/stdp.mod\n");
   ...
   if ( _lflag  == 0.0 ) {
     ...
     if ( learning ) {
       if ( _args[0] >= minWeight ) {
         _args[0] = _args[0] - LR * d * exp ( ( tpost - t ) / dtau ) ;
         if ( _args[0] <= minWeight ) {
           _args[0] = minWeight ;
           }
         if ( verbose ) {
 	 ...
 	 _fnargs1 = _fnargslist1[_ifn1];
 {
       if ( learning ) {
         if ( _fnargs1[0] <= maxWeight ) {
           _fnargs1[0] = _fnargs1[0] + LR * p * exp ( ( _fnargs1[1] - t ) / ptau ) ;
           if ( _fnargs1[0] > maxWeight ) {
